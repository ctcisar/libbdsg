...
#binder_executable = glob.glob(f'/home/ctcisar/git/binder/build/llvm-4.0.0/build_4.0.0.linux.SHODAN.release/bin/binder')[0]
binder_executable = f''
bindings_dir = 'cmake_bindings'
#binder_source = f'/home/ctcisar/git/binder/source'
#pybind_source = f'/home/ctcisar/git/binder/build/pybind11/include'
binder_source = f''
pybind_source = f''
use_pybind_stl = True
...
def clone_repos():
    if not os.path.exists(f'binder/pybind11'):
        git.Git("binder/pybind11").clone("https://github.com/RosettaCommons/pybind11.git")
    if not os.path.exists(f'binder/binder'):
        git.Git("binder/binder").clone("https://github.com/RosettaCommons/binder.git")
    pybind_source = f'binder/pybind11/include'
    binder_source = f'binder/binder/source'

def build_binder():
    if not os.path.exists(f'binder/binder/build'):
        subprocess.check_call(f'{get_python_inc()} binder/binder/build.py')
